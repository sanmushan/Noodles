...
public class NoodleDataUtil {
    ...
    public static NoodleTradeModel getData(InformationModle.MenuData menuData, int noodleStatus,int startNo) {
        ...
        if (noodleNo == 1) {
            price = Float.parseFloat(PreferenceUtil.config().getStringValue(Constants.RICE_PRICE));
        } else {
            price = Float.parseFloat(PreferenceUtil.config().getStringValue(Constants.NOODLE_PRICE));
        }
...
//            listModle.goods_no = riceND.noodleNo;
            ...
            if (noodleNo == 2) {
                ...
                switch (itemType) {
                    case 1:
                       // noodleName = DbTypeContants.QINGTANG_MAIN;
                        noodleState = NoodleNameConstants.QINGTANG;
                        riceRecipe = 1;
                        isStock = isStapleFood;
                        break;
                    case 2:
                       // noodleName = DbTypeContants.CHONGQING_MAIN;
                        riceRecipe = 2;
                        ...
                        break;
                    case 3:
                       // noodleName = DbTypeContants.LUDAN_MAIN;
                        riceRecipe = 3;
                        ...
                        break;
                    case 4:
                       // noodleName = DbTypeContants.JITUI_MAIN;
                        riceRecipe = 4;
                ...
                switch (itemType) {
                    case 1:
                      //  noodleName = DbTypeContants.QINGTANG_FENG;
                        riceRecipe = 1;
                        noodleState = NoodleNameConstants.QINGTANG;
                        isStock = isStapleFood;
                        break;
                    case 2:
                      //  noodleName = DbTypeContants.SUANLA_FENG;
                        riceRecipe = 2;
                        noodleState = NoodleNameConstants.CHONGQING;
                        isStock =isStapleFood&&(DBNoodleHelper.querynoodleStatusNoolde(DbTypeContants.SUANLABAO).size() > 0 ? true : false);
                        break;
                    case 3:
                        //noodleName = DbTypeContants.LUDAN_FENG;
                        riceRecipe = 3;
                        noodleState = NoodleNameConstants.LUDAN;
                        isStock = isStapleFood&&(DBNoodleHelper.querynoodleStatusNoolde(DbTypeContants.SUANLABAO).size() > 0 ? true : false) &&
                                (DBNoodleHelper.querynoodleStatusNoolde(DbTypeContants.LUJIDANG).size() > 0 ? true : false);
                        break;
                    case 4:
                       // noodleName = DbTypeContants.JITUI_FENG;
                        riceRecipe = 4;
                        noodleState = NoodleNameConstants.HAOHUA;
                        isStock = isStapleFood&&(DBNoodleHelper.querynoodleStatusNoolde(DbTypeContants.SUANLABAO).size() > 0 ? true : false) &&
                                (DBNoodleHelper.querynoodleStatusNoolde(DbTypeContants.SUANLAJITUI).size() > 0 ? true : false);
