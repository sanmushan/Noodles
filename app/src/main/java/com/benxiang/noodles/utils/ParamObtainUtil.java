...
//        List<PlaceOrderParam.OrderDetailDatas> orderDetailDatases = new ArrayList<>();
//        PlaceOrderParam.OrderDetailDatas orderDetailDatas = new PlaceOrderParam.OrderDetailDatas();
//        for (int i = 0; i < listModles.size(); i++) {
//            ListModle listModle = listModles.get(i);
//            double Price = FormatUtil.flaotToDouble(listModle.goods_prive);
//            orderDetailDatas.productId = listModle.productId;
//            orderDetailDatas.productName = listModle.goods_name;
//            orderDetailDatas.price = (float) Price;
//            orderDetailDatas.CostPrice = (float) Price;
//            orderDetailDatas.SourcePrice = (float) Price;
//            orderDetailDatas.DishSumReal = (float) Price;
//            orderDetailDatas.isSuite = 0;
////            orderDetailDatas.count = 1;
//            orderDetailDatas.count = listModle.goods_num;
////        orderDetailDatas.Remark = "多加点料";
//            orderDetailDatases.add(orderDetailDatas);
//        }
//        orderData.orderDetail = orderDetailDatases;
...
//                int zhaCaiNum = 0;
//                int luDanNum = 0;
//                ArrayList<RiceND> zhaCaiNDs = DBNoodleHelper.querynoodleStatusNoolde(DbTypeContants.SUANLAJITUI);// 鸡腿改成了榨菜，但最大的库存量不变
//                ArrayList<RiceND> luDanNDs = DBNoodleHelper.querynoodleStatusNoolde(DbTypeContants.LUJIDANG);
//                for (int i=0;i<zhaCaiNDs.size();i++){
//                    zhaCaiNum+=zhaCaiNDs.get(i).totalNum;
//                }
//                for (int i=0;i<luDanNDs.size();i++){
//                    luDanNum+=luDanNDs.get(i).totalNum;
//                }
//
...
//                BeginLotteryModel.SellOutIyem sellOutIyem = beginLotteryModel.new SellOutIyem();
//                sellOutIyem.MenuTypeCode = "99";
//                sellOutIyem.MenuItemCode = "9901";
//                sellOutIyem.MenuItemCName = "卤蛋";
//                sellOutIyem.stockNum = luDanNum;
//
//                BeginLotteryModel.SellOutIyem zhaCaiSellOutIyem = beginLotteryModel.new SellOutIyem();
//                zhaCaiSellOutIyem.MenuTypeCode = "99";
//                zhaCaiSellOutIyem.MenuItemCode = "9902";
//                zhaCaiSellOutIyem.MenuItemCName = "榨菜";
//                zhaCaiSellOutIyem.stockNum = zhaCaiNum;
//
//                list.add(sellOutIyem);
//                list.add(zhaCaiSellOutIyem);
//                //返回的数据
...
//            CloseLotteryModel closeLotteryModel = listModle.closeLotteryModels.get(0);

            double Price = FormatUtil.flaotToDouble(0);
//            orderDetail.productId = NoodleTradeFieldUtil.getProductID(closeLotteryModel.spoilName);
//            orderDetail.productName = closeLotteryModel.spoilName+"(赠)";
