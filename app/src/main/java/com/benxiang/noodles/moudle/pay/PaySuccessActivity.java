...
//        NoodleDataUtil.getOrderToDB(shotrNo, mNoodleTradeModel.listModles);
...
//        Timber.e("下单的参数:"+JsonHelper.getGson().toJson(placeOrderParam));
    ...
    private void ToRefundment(int payWay) {
       ...
       /* payParam.money =2 + "";
        payParam.orderNo = "080027584CFC201713050154";
        mPaySuccessPresenter.refundment("RunAlipayRefund", JsonHelper.getGson().toJson(payParam));*/
...
//        mNoodleTradeModel.order_No = strMsg.BillNo;
//        mNoodleTradeModel.order_No = PreferenceUtil.config().getStringValue("orderNo");
//        mNoodleTradeModel.take_meal_No = strMsg.BillNo;
  ...
  /*      for (int i=0;i<mNoodleTradeModel.listModles.size();i++) {
        ...
        RetreatFoodParam param = ParamObtainUtil.getRetreadFood(mNoodleTradeModel, mNoodleTradeModel.listModles);
        placeOrderPresenter.retreatFood(MethodConstants.BACKORDERITEM,JsonHelper.getGson().toJson(param));*/
        //下单完成后加菜
        ToAddOrderItem();
    }
